...
The answer is C because x is deleted with the setTimeout function. the order is 'z' then 'y' because console opperates on a zero index and the letter 'y' is given the index 1.
// -----------------------------------------------
...
function spanishColorUsingObject (spanishColName) {
  var colorObject = {
    rojo: '#ff0000',
    blanco: '#fffff',
    azul: '#0000ff',
  ...
  return colorObject[spanishColName];
}

spanishColorUsingObject ('rojo')
...
// ARRAY FOR STUDENTS RANTOM named

 Input = list of student names.
 Output = one random names.
 ...
 // array example
  ...
  function randomStudent (anyListOfStudents) {
    var randomNum = Math.random(); // number between zero and one
